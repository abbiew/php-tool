...
class MySQL implements \Swoole\IDatabase
{
    ...
    function connect()
    {
        ...
        if (!$this->conn)
        {
            Swoole\FwError::info(__CLASS__." SQL Error", mysql_error($this->conn));
            return false;
        }
        mysql_select_db($db_config['name'], $this->conn) or Swoole\FwError::info("SQL Error", mysql_error($this->conn));
        if ($db_config['setname'])
        {
            mysql_query('set names ' . $db_config['charset'], $this->conn) or Swoole\FwError::info("SQL Error",
    ...
    function query($sql)
    {
        ...
        for ($i = 0; $i < 2; $i++)
        {
            ...
            if ($res === false)
            {
                ...
                \Swoole\FwError::info(__CLASS__." SQL Error", $this->errorMessage($sql));
                return false;
            }
            break;
        }
